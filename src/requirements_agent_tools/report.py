...
def _build_report(slug: str, conn, meta) -> dict:
    ...
    for req in sorted(reqs, key=lambda r: r.updated_at, reverse=True)[:10]:
        updates = db.get_updates(conn, req.id)
        if updates:
            latest = updates[-1]
    ...
    recent_changes = sorted(recent_changes,
                            key=lambda x: x["changed_at"], reverse=True)[:5]
    ...
    return {
        ...
        "requirements": {
            ...
            "critical_open":  [r.id for r in reqs
        ...
        "decisions": {
            ...
            "open_items": [
                {"decision_id": d["decision_id"], "title": d["title"],
                 "made_by": d["made_by"], "action_owner": d.get("action_owner"),
...
def _report_to_md(report: dict) -> str:
    ...
    lines = [
        ...
        f"- By type: " + ", ".join(f"{k}: {v}" for k, v in sorted(reqs["by_type"].items())),
        f"- By status: " + ", ".join(f"{k}: {v}" for k, v in sorted(reqs["by_status"].items())),
    ]
    ...
    for c in r["recent_changes"]:
        lines.append(f"- [{c['req_id']}] {c['title']} — {c['summary']} ({c['changed_by']}, {c['changed_at'][:10]})")
...
def cmd_save(args):
    ...
    json_out.write_text(json.dumps({"ok": True, "report": report}, indent=2, default=str),
                        encoding="utf-8")

    _ok({"md": str(md_out), "json": str(json_out),
