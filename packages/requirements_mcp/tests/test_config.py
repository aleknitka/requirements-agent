...
def test_resolve_db_path_returns_fixed_default() -> None:
    """The DB path is locked to ./data/requirements.db (absolute form)."""
    assert resolve_db_path() == DEFAULT_DB_PATH.expanduser().resolve()


def test_resolve_db_path_ignores_env_var(monkeypatch: pytest.MonkeyPatch) -> None:
    """``REQUIREMENTS_DB_PATH`` is documented but no longer overrides."""
    monkeypatch.setenv(ENV_VAR, "/tmp/should-be-ignored.db")
    assert resolve_db_path() == DEFAULT_DB_PATH.expanduser().resolve()


def test_resolve_db_path_returns_absolute() -> None:
