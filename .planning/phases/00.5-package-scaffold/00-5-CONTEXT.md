...
- **D-09:** Add `lazydocs` as a dev dependency: `uv add --dev lazydocs`. (Revised 2026-04-28: switched from pdoc — pdoc cannot generate Markdown; user chose lazydocs which generates GitHub-flavored Markdown.)
- **D-10:** Generate **Markdown** output only (lazydocs native format). No HTML output.
...
- **D-15:** `lazydocs` doc regeneration — regenerate `docs/` on every commit so docs stay in sync.
...
### Integration Points
- `pyproject.toml` — add dev deps (`lazydocs`, `pre-commit`, `interrogate`, `bandit`, `ty`) and a `[tool.interrogate]` config section
- `.pre-commit-config.yaml` — new file; runs: `pre-commit-hooks` (detect-private-key), ruff, ty, bandit, interrogate, lazydocs, pytest
